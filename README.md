...
- **Simulates memory access delay for instructions, data and page table walk** via three direct-mapped TLBs (for code, loads and stores), two-level cache hierarchy with various allocation and miss handling policies and two DRAM memory models: Simplistic base DRAM model that simulates open-page hits and [DRAMSim3](https://github.com/umd-memsys/DRAMSim3)
...
| `-sim-mem-model`            | `base`, `dramsim3` | To specify which memory model to use, run with command line option `-sim-mem-model` and specify either `base` or `dramsim3`. The default is `base`. For DRAMSim3, the paths to `config file` can be specified in the config file.                                                  |
...
| `-sim-file-path`            | `directory path`   | Path of the directory to store stats, log, dramsim3 stats, and trace files. Default is current directory `.`, the user must create the directory before starting MARSS-RISCV.                                                                                                                                     |
